...
def main() -> None:
    ...
    parser.add_argument("--model-id", default="claude-3-5-sonnet-latest", help="Claude model ID")
    parser.add_argument("--db-path", default="core.db", help="SQLite database path")
    args = parser.parse_args()

    store = SQLiteStore(db_path=args.db_path)
    session_manager = SessionManager(store=store, default_model_id=args.model_id)
    session_id, model_id, state = session_manager.start_or_resume(args.session_id, args.model_id)
    ...
    interview_agent = InterviewAgent(DEFAULT_POLICY)
    ...
    def on_turn_complete(updated_state: dict) -> None:
        ...
        session_manager.save_state(session_id, model_id, updated_state, completed=False)

    def on_completed(updated_state: dict) -> None:
        report = updated_state.get("final_report", {})
        store.save_report(session_id=session_id, report=report)
        session_manager.save_state(session_id, model_id, updated_state, completed=True)
